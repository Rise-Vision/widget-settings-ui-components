...
(function() {
  ...
  describe("Position Setting Component", function() {
    ...
    it("Should correctly load", function () {
      expect(element(by.css("select[name='position']")).isPresent()).to.eventually.be.true;
      expect(element(by.css("select[name='position']")).isDisplayed()).to.eventually.be.true;
    });

    it("Should correctly load with default selection", function () {
      element.all(by.css("select[name='position'] option")).then(function (elements) {
      ...
      expect(element(by.css("select[name='position']")).$('option:checked').getAttribute("value"))
    ...
    it("Should update the position model value by selecting a different option", function () {
      element(by.css("select[name='position']")).click();
      element(by.css("select[name='position'] option[value='middle-right']")).click();

      // ng-model 'position' value should be bound to select element and have the correct value
      expect(element(by.model("position")).getAttribute("value")).to.eventually.equal("middle-right");
    })
